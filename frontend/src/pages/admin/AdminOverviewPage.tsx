    ...
    return (
      <div className="p-8 space-y-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
          {[1, 2, 3, 4, 5].map((i) => (
            <div key={i} className="h-32 bg-white rounded-2xl border border-slate-200 shadow-sm animate-pulse" />
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div className="p-6 sm:p-8 space-y-8 bg-slate-50 min-h-screen">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-extrabold text-slate-900 tracking-tight">Institutional Overview</h2>
        ...
        <div className="lg:col-span-5 bg-white border border-slate-200 rounded-3xl p-6 shadow-sm flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-bold text-slate-900 tracking-tight uppercase flex items-center space-x-2">
            ...
            <div className="space-y-3">
              {metrics?.violation_breakdown && Object.keys(metrics.violation_breakdown).length > 0 ? (
                ...
                <div className="text-center py-10 space-y-2">
                  ...
                  <p className="text-xs text-slate-500">No violations recorded. 100% integrity baseline maintained.</p>
                </div>
              )}
            </div>
          </div>

          <div className="mt-6 pt-3 border-t border-slate-200 flex items-center justify-between text-[11px] text-slate-500 font-mono">
        ...
        <div className="lg:col-span-7 bg-white border border-slate-200 rounded-3xl p-6 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center space-x-2">
              <h3 className="text-sm font-bold text-slate-900 tracking-tight uppercase">Live Candidate Sessions</h3>
                  ...
                  metrics.recent_attempts.slice(0, 6).map((att) => {
                    ...
                    return (
                      <tr key={att.id} className="hover:bg-slate-50/70 transition-colors">
                        ...
                        <td className="p-2.5 text-slate-700 max-w-[130px] truncate">{att.exam_title}</td>
                        <td className="p-2.5">
                          {isInProgress ? (
                            <span className="inline-flex items-center space-x-1 px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider bg-emerald-50 text-emerald-700 border border-emerald-200 animate-pulse">
                              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
                              <span>Live</span>
                            </span>
                          ) : isTerminated ? (
                            <span className="text-[10px] font-bold text-rose-700 uppercase bg-rose-50 border border-rose-200 px-2 py-0.5 rounded-md">
                              Terminated
                            </span>
                          ) : (
                            ...
                            {isInProgress && (
                              <>
                                <button
                                  ...
                                  className="flex items-center space-x-1 px-2 py-1 bg-blue-50 hover:bg-blue-600 text-blue-700 hover:text-white border border-blue-200 rounded-lg text-xs font-bold transition-colors cursor-pointer"
